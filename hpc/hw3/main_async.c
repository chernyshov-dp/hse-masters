...
double *next_temp_distribution(double *temp, const int k, const double h, const double tau, const int rank, const int num_proc, const int frags_per_proc, MPI_Request *requests)
{
    ...
    if (rank != 0)
    {
        MPI_Irecv(&left_el, 1, MPI_DOUBLE, rank - 1, 10, MPI_COMM_WORLD, &req[0]);
    }

    if (rank != num_proc - 1)
    {
        MPI_Irecv(&right_el, 1, MPI_DOUBLE, rank + 1, 11, MPI_COMM_WORLD, &req[1]);
    }

    if (rank != 0)
    {
        MPI_Isend(temp, 1, MPI_DOUBLE, rank - 1, 11, MPI_COMM_WORLD, &req[2]);
    }

    if (rank != num_proc - 1)
    {
        MPI_Isend(temp + frags_per_proc - 1, 1, MPI_DOUBLE, rank + 1, 10, MPI_COMM_WORLD, &req[3]);
    }
    ...
    {
        double ul = (i != 0) ? temp[i - 1] : left_el;
        double ur = (i != frags_per_proc - 1) ? temp[i + 1] : right_el;
        double u = temp[i];
...
int main(int argc, char **argv)
{
    ...
    {
        next_temp_distribution(temp_reg, k, h, tau, rank, num_proc, frags_per_proc, requests);
        MPI_Barrier(MPI_COMM_WORLD);
    }
